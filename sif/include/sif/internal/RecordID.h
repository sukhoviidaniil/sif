...
namespace sif::intrnl {
    struct RecordID {
        ...
        // equality
        friend constexpr bool operator==(const RecordID& a, const RecordID& b) noexcept { return a.id == b.id; }
    ...
    struct RecordIDHash {
        size_t operator()(const RecordID& g) const noexcept { return std::hash<std::uint32_t>{}(g.id); }
    };
} // namespace sif::intrnl
