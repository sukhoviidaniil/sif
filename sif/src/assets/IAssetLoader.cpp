...
namespace sif::asset {
    ...
    void IAssetLoader::try_load(AssetRecord& record, const std::string& asset_dir) {
        ...
        for (int attempt = 1; attempt <= max_attempts; ++attempt) {
            LOG("Start loading asset: " + asset_name + " (attempt " + std::to_string(attempt) + ", budget " +
            ...
            try {
                ...
                if (!record.set_state_if_current(attempt_token, AssetState::Failed)) {
                    LOG("Discarded stale failure for '" + asset_name + "': a newer load attempt has since started.");
                }
            ...
            if (elapsed_seconds <= allowed_seconds) {
                if (record.set_state_if_current(attempt_token, AssetState::Ready)) {
                    LOG("Successfully loaded asset: " + asset_name + " in " + std::to_string(elapsed_seconds) + "s");
                } else {
                    LOG("Discarded stale success for '" + asset_name + "': a newer load attempt has since started.");
                }
                return;
            }

            // The load succeeded, but broke its promised time budget.
            const std::string warning = "Asset '" + asset_name + "' exceeded its promised load time (" +
                                        std::to_string(elapsed_seconds) + "s > " + std::to_string(allowed_seconds) +
                                        "s).";
            LOG(warning);

            if (attempt >= max_attempts) {
                const std::string err = "Asset '" + asset_name + "' failed to load within its doubled time budget (" +
                                        std::to_string(allowed_seconds) + "s).";
                if (record.set_state_if_current(attempt_token, AssetState::Failed)) {
                    LOG(err);
                } else {
            ...
            allowed_seconds *= 2.0;
            LOG("Retrying asset '" + asset_name + "' with doubled time budget: " + std::to_string(allowed_seconds) +
                "s");
        }
    }
} // namespace sif::asset
