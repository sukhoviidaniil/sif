...
namespace sif::asset::data {
    ...
    namespace {
         ...
         * registry uses).
         */
        void patch_meta_in_file(const std::filesystem::path& path, const data::AssetMetaData& meta,
    ...
    std::unique_ptr<PrimitiveAnimationNode>
    AssetDataLoader::load_PrimitiveAnimation_from_file(const std::filesystem::path& path) {
        const nlohmann::json& j = io::get_json_data(path);
