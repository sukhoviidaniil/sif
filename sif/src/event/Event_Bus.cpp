...
namespace sif::event {
    ...
    Event_Bus::Subscription::Subscription(std::shared_ptr<Event_Bus> bus, const std::type_index type,
                                          const HandlerId id)
    ...
    void Event_Bus::unsubscribe(std::type_index type, HandlerId id) {
        ...
        auto& list = it->second;
        list.erase(std::remove_if(list.begin(), list.end(), [id](const Handler& h) { return h.id == id; }), list.end());
    }

    void Event_Bus::sort(std::vector<Handler>& list) {
        std::sort(list.begin(), list.end(), [](const Handler& a, const Handler& b) { return a.priority > b.priority; });
    }
