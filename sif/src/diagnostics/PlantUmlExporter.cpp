...
namespace {
    ...
    const std::vector<std::string>& colorPalette() {
        static const std::vector<std::string> palette = {"#E6194B", "#3CB44B", "#4363D8", "#F58231", "#911EB4",
...
namespace sif::diag {
    ...
    std::string exportToPlantUml(const math::DirectedGraph<std::string>& graph, const DeclarationGraphResult& metadata,
                                 const std::vector<std::vector<VertexId>>& cycles, const std::string& outputFolder,
        ...
        for (VertexId from : graph.vertexIds()) {
            ...
            for (VertexId to : graph.outNeighbors(from)) {
                ...
                bool sameCycle = fromCycleIt != vertexToCycleIndex.end() && toCycleIt != vertexToCycleIndex.end() &&
