...
namespace sif::intrnl {
    ...
    Color::Color(std::string_view s, std::string separator) {
        ...
        if (count != 3 && count != 4) {
            throw std::invalid_argument("Color must be r" + separator + "g" + separator + "b or r" + separator + "g" +
                                        separator + "b" + separator + "a");
        }

        r = static_cast<std::uint8_t>(values[0]);
        g = static_cast<std::uint8_t>(values[1]);
        b = static_cast<std::uint8_t>(values[2]);
        a = (count == 4) ? static_cast<std::uint8_t>(values[3]) : static_cast<std::uint8_t>(255);
    }
} // namespace sif::intrnl
